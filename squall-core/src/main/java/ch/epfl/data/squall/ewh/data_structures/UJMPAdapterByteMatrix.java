...
// Join Attribute Type
public class UJMPAdapterByteMatrix<JAT extends Comparable<JAT>> extends
		JoinMatrix<JAT> {
	public static void main(String[] args) {
		UJMPAdapterByteMatrix<Integer> joinMatrix = new UJMPAdapterByteMatrix<Integer>(
				100, 100);
	...
	public UJMPAdapterByteMatrix(int xSize, int ySize) {
		_capacity = SystemParameters.MATRIX_CAPACITY_MULTIPLIER
				* (xSize + ySize);
		_ujmpMatrix = new DefaultSparseByteMatrix(_capacity, new long[] {
				xSize, ySize }); // The first argument is MAX_SIZE
	...
	public UJMPAdapterByteMatrix(String matrixPath, String matrixName) {
		...
		try {
			...
			_ujmpMatrix = new DefaultSparseByteMatrix(
	...
	public UJMPAdapterByteMatrix<JAT> getDeepCopy() {
		if (_capacity == -1) {
			...
			throw new RuntimeException(
					"Cannot make a deep copy of UJMPAdapterByteMatrix which created from a file!");
		}
