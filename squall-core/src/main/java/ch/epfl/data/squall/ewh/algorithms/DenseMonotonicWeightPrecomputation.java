...
import ch.epfl.data.squall.ewh.data_structures.SparseMatrixUJMP;
...
public class DenseMonotonicWeightPrecomputation implements WeightPrecomputation {
	private static Logger LOG = Logger
			.getLogger(DenseMonotonicWeightPrecomputation.class);
	...
	public DenseMonotonicWeightPrecomputation(WeightFunction wf,
			JoinMatrix joinMatrix, Map map) {
		_map = map;
		_wf = wf;
	...
	public DenseMonotonicWeightPrecomputation(WeightFunction wf,
			JoinMatrix joinMatrix, Map map, ShallowCoarsener sc) {
		_map = map;
		_wf = wf;
		_joinMatrix = joinMatrix;
		_sc = sc;
	...
	private void choosePrefixSumType() {
		...
		if (prefixSumType.equalsIgnoreCase("INT_INT")) {
			...
			int numCandidateRoundedCells = _sc
					.getNumCandidateRoundedCells(_map);
			...
			if (_sc instanceof InputShallowCoarsener) {
				...
				// roundedCellArea differs from cell to cell; we multiply with
				// an arbitrary constant
				LOG.info("Should be here only when IIPrecDense or IIPrecBoth");
				roundedCellArea *= 4;
			}
			int capacity = numCandidateRoundedCells * roundedCellArea;
			LOG.info("PrefixSumType is UJMP: Capacity is " + capacity + " = "
					+ numCandidateRoundedCells + " * " + roundedCellArea
					+ " (maybe some multiplier).");
			_prefixSum = new SparseMatrixUJMP(capacity, _xSize, _ySize);
		} else {
			throw new RuntimeException("Unsupported prefixSumType = "
					+ prefixSumType);
		}
	}
	...
	private int computePrefixUp(int originalXBeginBucket, int originalX,
			int originalY) {
		if (originalX == originalXBeginBucket) {
			...
			// Due to monotonicity, cannot be left from the first candidate from
			// the above row
	...
	private void precompute() {
		...
		if (_sc == null) {
			// if not sent through constructor, we build a new
			// InputShallowCoarsener

			// more efficient for PREFIX_SUM_TYPE = INT_INT: only small portion
			// of prefixSum elements are candidates, and thus is assigned a
			// non-zero value
			...
			if (SystemParameters.isExisting(_map, "PRECOMPUTATION_DIVIDER")) {
				divider = SystemParameters.getInt(_map,
						"PRECOMPUTATION_DIVIDER");
			...
			// ensures that the last bucket is at most 2 * 2 = 4 times bigger
			// than other buckets
			numXBuckets = MyUtilities.adjustPartitioning(_xSize, numXBuckets,
					"p_i_x");
			numYBuckets = MyUtilities.adjustPartitioning(_ySize, numYBuckets,
					"p_i_y");
		...
		for (int i = 0; i < _sc.getNumXCoarsenedPoints(); i++) {
			...
			for (int j = firstCandInLastLine; j < _sc.getNumYCoarsenedPoints(); j++) {
				...
				boolean isCandidate = MyUtilities.isCandidateRegion(
						_joinMatrix, region,
						_joinMatrix.getComparisonPredicate(), _map);
				if (isCandidate) {
		...
		if (_prefixSum.getNumElements() > _prefixSum.getCapacity()) {
			throw new RuntimeException("Cannot put "
					+ _prefixSum.getNumElements() + " into matrix of capacity "
					+ _prefixSum.getCapacity());
		}
	}
