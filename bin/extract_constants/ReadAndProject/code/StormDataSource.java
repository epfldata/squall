...
public class StormDataSource extends BaseRichSpout implements StormEmitter, StormComponent {
	...
	private long _batchOutputMillis;
        ...
        private long TUPLE_READ = 100000;
		...
		public void nextTuple() {
			if(_firstTime && MyUtilities.isBatchOutputMode(_batchOutputMillis)){
				...
				_firstTime = false;
                        ...
                        if(_numSentTuples % TUPLE_READ == 0){
                            long currentTime = System.currentTimeMillis();
                            if(_numSentTuples != 0){
				long difference = currentTime - _readTimestamp;
                                LOG.info("The timeout for reading " + TUPLE_READ + " tuples is " + difference + "ms.");
				LOG.info("  (which is " + ((double)difference)/TUPLE_READ + "ms per a tuple.");
                            }
                            _readTimestamp = currentTime;
                        }
