...
public class DBToasterGoogleGoodnessScoreSequentialPlan extends QueryPlan {
    private static Logger LOG = Logger.getLogger(DBToasterGoogleGoodnessScoreSequentialPlan.class);
    ...
    private final int EVENT_EVICT = 2;
    private final int EVENT_FAILED = 3;
    private final int EVENT_FINISH = 4;
    private final int EVENT_LOST = 6;

    private final QueryBuilder _queryBuilder = new QueryBuilder();

    public DBToasterGoogleGoodnessScoreSequentialPlan(String dataPath, String extension, Map conf) {
        ...
        ComparisonPredicate evict = new ComparisonPredicate(
            ComparisonPredicate.EQUAL_OP, new ColumnReference(_ic, 5),
                        new ValueSpecification(_ic, EVENT_EVICT));

        ComparisonPredicate failed = new ComparisonPredicate(
            ComparisonPredicate.EQUAL_OP, new ColumnReference(_ic, 5),
                        new ValueSpecification(_ic, EVENT_FAILED));

        ComparisonPredicate lost = new ComparisonPredicate(
            ComparisonPredicate.EQUAL_OP, new ColumnReference(_ic, 5),
                        new ValueSpecification(_ic, EVENT_LOST));
        
        OrPredicate orTasks = new OrPredicate(failed, lost, evict);

        SelectOperator selectionKilledTasks = new SelectOperator(orTasks);
