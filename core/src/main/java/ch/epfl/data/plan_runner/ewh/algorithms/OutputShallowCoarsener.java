...
public class OutputShallowCoarsener extends ShallowCoarsener {
	protected enum ADJUST_MODE {
		...
		// but "adjust to numBuckets" partitions the smaller relation more
		// nicely
		// "backup-restore" is exactly how they say the algorithm should look
		// like;
		// one can think of "adjust to numBuckets" as an optimization
		BACKUP_RESTORE, ADJUST_NUM_BUCKETS // BETTER
		// Another way: Given list of weights, iteratively:
		// * extract those single-liners which have weight > avg; assign them to
		// a single bb
		...
		// TODO: maybe the right way to do this would be to modify the
		// algorithm,
		// such that we multiply by some factor proportionally to (originalXSize
		// - numXBuckets)
	}
	...
	public static void main(String[] args) throws IOException {
		...
		JoinMatrix joinMatrix = new UJMPAdapterByteMatrix(matrixRoot,
				matrixName);
		...
		OutputShallowCoarsener coarsener = new OutputShallowCoarsener(50, 50,
				new WeightFunction(1, 1), null, amode, imode);
	...
	/*
	 * // the same as in input private void
	 * computeCoarsenedMatrix(JoinBooleanMatrixInterface originalMatrix,
	 * StringBuilder sb) { _rbListX = new ArrayList<BucketBoundary>(); _rbListY
	 * = new ArrayList<BucketBoundary>();
	 * 
	 * // x int bucketXSize = _originalXSize / _numXBuckets; for(int i = 0 ; i <
	 * _numXBuckets - 1; i++){ BucketBoundary bb = new BucketBoundary(i *
	 * bucketXSize, (i+1) * bucketXSize - 1); _rbListX.add(bb); } BucketBoundary
	 * bb = new BucketBoundary((_numXBuckets - 1) * bucketXSize, _originalXSize
	 * - 1); _rbListX.add(bb);
	 * 
	 * // y int bucketYSize = _originalYSize / _numYBuckets; for(int i = 0 ; i <
	 * _numYBuckets - 1; i++){ bb = new BucketBoundary(i * bucketYSize, (i+1) *
	 * bucketYSize - 1); _rbListY.add(bb); } bb = new
	 * BucketBoundary((_numYBuckets - 1) * bucketYSize, _originalYSize - 1);
	 * _rbListY.add(bb); }
	 */

	public OutputShallowCoarsener(int numXBuckets, int numYBuckets,
			WeightFunction wf, Map map) {
		this(numXBuckets, numYBuckets, wf, map, ADJUST_MODE.ADJUST_NUM_BUCKETS,
				ITER_MODE.MAX_ITER);
		// this(numXBuckets, numYBuckets, wf, map,
		// ADJUST_MODE.ADJUST_NUM_BUCKETS, ITER_MODE.MAX_WEIGHT);
	}
	...
	public OutputShallowCoarsener(int numXBuckets, int numYBuckets,
		...
		if (SystemParameters.isExisting(map, "ROUNDING_OUTPUT_ITER_MULT")) {
			ITER_MULTIPLIER = SystemParameters.getInt(map,
					"ROUNDING_OUTPUT_ITER_MULT");
			LOG.info("Setting ROUNDING_OUTPUT_ITER_MULT to " + ITER_MULTIPLIER);
		}
	}

	private void addSplit(List<Double> weights, List<BucketBoundary> result,
			BucketBoundary bbSplit) {
	...
	private List<BucketBoundary> adjustToNumBuckets(List<Double> weights,
			List<BucketBoundary> bbList, int numBuckets) {
		 ...
		 * LOG.info("bucketsToSplit " + bucketsToSplit); LOG.info("bbList.size "
		 * + bbList.size()); LOG.info("numBuckets " + numBuckets);
		 */
	...
	/*
	 * Partitions row/columnWeights into p/q partitions A2A
	 */
	private void computeAlphaGoodPartitioning(int iteration) {
		...
		_bbListX = computeAlphaGoodPartitioning(_rowWeights, _rowWeightsSum,
				_numXBuckets, _originalXSize, _bbListX, iteration);
		_bbListY = computeAlphaGoodPartitioning(_columnWeights,
				_columnWeightsSum, _numYBuckets, _originalYSize, _bbListY,
				iteration);
	...
	/*
	 * According to the paper Do not allow weight of a tile to go over the
	 * average The best case for load-balancing may be to slightly better than
	 * the average But then we might end up having less buckets than required
	 */
	private List<BucketBoundary> computeAlphaGoodPartitioning(
			List<Double> weights, double weightsSum, int numBuckets,
			int originalSideSize, List<BucketBoundary> bbList, int iteration) {
		...
		/*
		 * if(iteration == 358){ LOG.info("maxSumRowWeight " + maxSumRowWeight);
		 * LOG.info("weights.size() " + weights.size()); LOG.info("numBuckets "
		 * + numBuckets); LOG.info("originalSideSize " + originalSideSize); }
		 */
		...
		for (int i = 0; i < originalSideSize; i++) {
			...
			if (currentSumRowWeight >= maxSumRowWeight) {
				...
				if (i == previousDivider) {
					// a single actual row surpasses the maximum; each rowTile
					// must take at least one actual row
					bb = new BucketBoundary(previousDivider, i,
							currentSumRowWeight);
					previousDivider = i + 1;
				} else {
					bb = new BucketBoundary(previousDivider, i - 1,
							currentSumRowWeight - weights.get(i));
					previousDivider = i;
				...
				if (bbList.size() == numBuckets - 1) {
					// last entry is added separately; it may be bigger than
					// others
					break;
				}
			}
		...
		if (previousDivider <= originalSideSize - 1) {
			currentSumRowWeight = getSumElements(weights, previousDivider,
					originalSideSize - 1);
			BucketBoundary bb = new BucketBoundary(previousDivider,
					originalSideSize - 1);
			bbList.add(bb);
		}
		...
		if (_amode == ADJUST_MODE.ADJUST_NUM_BUCKETS) {
			...
			if (bbList.size() != numBuckets) {
				throw new RuntimeException("The number of generated buckets "
						+ bbList.size()
						+ " differ from the number of required buckets "
						+ numBuckets + "!");
			}
		}
	...
	/*
	 * This is implementation of pxq algorithm; Given p,q, we try to build best
	 * matrix (one with the smallest maxWeight GridCell region)
	 */
	private void computeCoarsenedMatrix(StringBuilder sb) {
		long start = System.currentTimeMillis();
		if (SystemParameters.MONOTONIC_PRECOMPUTATION) {
			_wp = new DenseMonotonicWeightPrecomputation(_wf, _originalMatrix,
					_map);
		} else {
		...
		if (_imode == ITER_MODE.MAX_WEIGHT) {
			...
			LOG.info("We aim to get maxGridCellWeight = " + maxGridCellWeight
					+ " and it takes " + elapsed + " seconds.");
		...
		while (true) {
			...
			if (_imode == ITER_MODE.MAX_WEIGHT) {
				...
				if (chosenRegion == null) {
					// There is no candidate region in the rounded matrix
					// spanning on at least 2 sample matrix cells
					 ...
					 * LOG.info("WARNING! WARNING! WARNING! WARNING! "); break;
					 */
				}
			}
		...
		if (_imode == ITER_MODE.MAX_ITER) {
			// continue with bucket boundaries which led to minimum maximum
			// region weight (weight in the work/BSP sense)
		...
		LOG.info("Part of BSP algorithm: Precomputation takes "
				+ elapsedPrecomputation
				+ " seconds, and rounding pxp algorithm takes "
				+ elapsedRounding + " seconds.");
		sb.append("\nPrecomputation takes ").append(elapsedPrecomputation)
				.append(" seconds, " + "and rounding pxp algorithm takes ")
				.append(elapsedRounding).append(" seconds.\n");
	}
	...
	protected double computeMaxGridCellWeight() {
		...
		double result = MAX_WEIGHT_BADNESS * (2 + EPSILON)
				* _originalMatrix.getTotalNumOutputs() / minSideLength;
			...
			for (int j = firstCandInLastLine; j < _bbListY.size(); j++) {
				...
				if (isCandidate) {
					if (region.getHalfPerimeter() > 2 && weight > maxWeight) {
						// we can reduce the size of region only if it is bigger
						// than 1x1 (sample matrix size)
						maxWeight = weight;
						maxRegion = region;
					}
					if (isFirstInLine) {
	...
	private Region findOverweighted(double maxGridCellWeight, int iteration) {
		...
		for (int i = 0; i < _bbListX.size(); i++) {
			...
			for (int j = firstCandInLastLine; j < _bbListY.size(); j++) {
				...
				boolean isCandidate = (weight > 0);
				if (isCandidate) {
					if (region.getHalfPerimeter() > 2
							&& weight > maxGridCellWeight) {
						// we can reduce the size of region only if it is bigger
						// than 1x1 (sample matrix size)
						return region;
					}
	...
	public int getCoarsenedXCoordinate(int x) {
		if (_originalToCoarsenedX.containsKey(x)) {
			...
			// DenseMonotonicWeightPrecomputation.precompute invokes
			// computePrefix invokes computePrefixUp invokes
			// _sc.getCoarsenedXCoordinate
			// In the second precomputation, _sc is OutputShallowCoarsener
			// Should never be in the else branch of
			// OutputShallowCoarsener.getCoarsenedYCoordinate, unless
			// secondPrecomputation is DENSE or BOTH
			int lowerBound = 0;
	...
	/*
	 * The following 6 methods are called from outside, when this algorithm is
	 * completed (has numXBuckets x numYBuckets)
	 */
	@Override
	...
	private List<Region> getSeparators() {
		...
		for (int i = 0; i < _bbListX.size(); i++) {
			...
			Region region = new Region(upperPos, 0, upperPos,
					_originalYSize - 1);
			regions.add(region);
		}
		...
		for (int j = 0; j < _bbListY.size(); j++) {
			...
			Region region = new Region(0, upperPos, _originalXSize - 1,
					upperPos);
			regions.add(region);
		}
	...
	private double multiplyElement(List<Double> weights, int position,
			double multiplier) {
	...
	private void multiplyWeights(Region overweighted) {
		...
		if (_originalXSize > _numXBuckets) {
			...
			if (x1 != x2) {
				// if the bucket is only 1 line wide, we cannot divide it
				// further
				for (int i = x1; i <= x2; i++) {
		...
		if (_originalYSize > _numYBuckets) {
			...
			if (y1 != y2) {
				...
				// further
				for (int j = y1; j <= y2; j++) {
					double deltaWeight = multiplyElement(_columnWeights, j,
							beta);
	...
	private void precomputeOriginalToCoarsened(List<BucketBoundary> bbList,
			Map<Integer, Integer> originalToCoarsened) {
		for (int i = 0; i < bbList.size(); i++) {
	...
	private void restoreY() {
		...
		for (int i = _backupColumnLower; i <= _backupColumnUpper; i++) {
			_columnWeights.set(i,
					_columnBackupWeights.get(i - _backupColumnLower));
		}
	}
	...
	private void setIModeConf() {
		if (SystemParameters.isExisting(_map, "ROUNDING_MODE")) {
			...
			if (rmode.equalsIgnoreCase("MAX_ITER")) {
				...
				throw new RuntimeException("Unrecognized ROUNDING_MODE = "
						+ rmode);
			}
		}
	}
	...
	public String toString() {
		return "OutputBinaryCoarsener [numXBuckets = " + _numXBuckets
