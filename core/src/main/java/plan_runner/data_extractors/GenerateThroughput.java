...
/**
 * 
 * This class reads the input "Results.csv" whose format is
 * tuples, percentage ,time (sec) 
 * and convert its into Through.csv whose format is
 * time (secs), throughput (tuples/sec), result% 
 * 
 */
public class GenerateThroughput {
	...
	private static final int SAMPLE_FREQ_SECS = 1; // check the code if you change the constant, it should be divided
	...
	void process() throws Exception{
	    ...
	    while ((nextLine = reader.readNext()) != null) {
	    	...
	    	for (int i = 0; i < nextLine.length/3; i++){
	    		if(!nextLine[3*i].equals("")&& (!nextLine[3*i].equals(" ")))
//	    			System.out.println("Line"+i+":"+nextLine[3*i]+","+nextLine[3*i+1]+","+nextLine[3*i+2]);
	    			
	    			try {
	    				allData.add(new csvThroughputObject(Long.parseLong(nextLine[3*i]), Double.parseDouble(nextLine[3*i + 1]), Double.parseDouble(nextLine[3*i + 2])));	
					} catch (Exception e) {
...
//    			if(!Double.isNaN(avgOutput)){
    				avgWithInSubinterval.add(new csvThroughputObject(avgOutput, avgPercentage, prevTime));
...
//    				}
    	...
    	csvThroughputObject prev= (csvThroughputObject)avgWithInSubinterval.get(0);
    	for (int i = 1; i < avgWithInSubinterval.size(); i++) {
    		...
    		double time=(prev._time+(current._time-prev._time)/2); // at that time
    		double resultPercentage=(prev._resultPercentage+(current._resultPercentage-prev._resultPercentage)/2); // at that %
    		
    		out.write(time+","+throughput+","+resultPercentage+'\n');
    		System.out.println(time+","+throughput+","+resultPercentage);
    	...
    	// maybe overestimate because within a subinterval all samples can have exact same TS
    	double time = avgWithInSubinterval.get(nextToLast)._time + SAMPLE_FREQ_SECS;
	...
	public class csvThroughputObject implements Comparable<csvThroughputObject>{
		...
		public csvThroughputObject(long numOutput ,double resultPercentage, double time) {
		...
		public int compareTo(csvThroughputObject o) {
			/*
			if(_numOutput > o._numOutput) return 1;
			else if (_numOutput == o._numOutput) return 0;
			else  return -1;
			*/
			if(_time > o._time) return 1;
			else if (_time == o._time) return 0;
			else  return -1;
		}
		@Override
	...
	public static void cleanUp(String inPath, String outPath,boolean removeLastChar) throws Exception{
	...
	public static void downSample(String path, double percentageToKeep, int steps) throws Exception{
		...
		ArrayList<String> buffer= new ArrayList<String>();
			...
			{
				val =rnd.nextDouble();
				if( ( steps>=0 && stepCounter%steps==0)|| (percentageToKeep>=0 && val<=percentageToKeep) ){
					buffer.add(text);
	...
	public static void main(String[] args) {
		 ...
		 */
			...
			*/
			...
			downSample(current+"csv/band_input/static_opt/Through.csv", -1, 2);
			downSample(current+"csv/band_input/dynamic/Through.csv", -1, 2);
			downSample(current+"csv/band_input/static_naive/Through.csv", -1, 2);
			
			downSample(current+"csv/theta_tpch5/static_opt/Through.csv", -1, 2);
			downSample(current+"csv/theta_tpch5/dynamic/Through.csv", -1, 2);
			downSample(current+"csv/theta_tpch5/static_naive/Through.csv", -1, 2);
			
			downSample(current+"csv/theta_tpch7/static_opt/Through.csv", -1, 2);
			downSample(current+"csv/theta_tpch7/dynamic/Through.csv", -1, 2);
			downSample(current+"csv/theta_tpch7/static_naive/Through.csv", -1, 2);
			...
			*/
			 ...
			 */
					/*
//			new GenerateThroughput(current+"csv/16Joiners/band_input/static_opt/");
			new GenerateThroughput(current+"csv/16Joiners/band_input/static_naive/");
			new GenerateThroughput(current+"csv/16Joiners/band_input/dynamic/");
			
			new GenerateThroughput(current+"csv/16Joiners/theta_tpch5/static_opt/");
			new GenerateThroughput(current+"csv/16Joiners/theta_tpch5/static_naive/");
			new GenerateThroughput(current+"csv/16Joiners/theta_tpch5/dynamic/");
			
			new GenerateThroughput(current+"csv/16Joiners/theta_tpch7/static_opt/");
			new GenerateThroughput(current+"csv/16Joiners/theta_tpch7/static_naive/");
			new GenerateThroughput(current+"csv/16Joiners/theta_tpch7/dynamic/");
			*/
		} catch (Exception e) {
			e.printStackTrace();
		}
