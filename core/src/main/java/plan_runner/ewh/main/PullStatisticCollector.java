...
public class PullStatisticCollector {
	...
	private static List<String> readTuple(CustomReader reader, ChainOperator operators, Map map){
		String line = null;
	...
	private static <T extends Comparable<T>> T readTupleJoinKey(CustomReader reader, ChainOperator operators, 
	...
	private static <T extends Comparable<T>> List<T> readAllTupleJoinKeys(CustomReader reader, ChainOperator operators, 
			TypeConversion<T> conv, Map map){
	...
	private void processConfig(Map map) {
        ...
        LOG.info("Elapsed time with visualization and writing to file for " + queryName + 
        		" of size [" + joinMatrix.getXSize() + ", " + joinMatrix.getYSize() + "]" +
        		" is " + elapsed + " seconds.");
	}
	
	public PullStatisticCollector(String args[]) {
		String configDirectory = args[0];
		List<String> configPaths = MyUtilities.listFilesForPath(configDirectory);
		LOG.info("There are " + configPaths.size() + " config files to process.");
	...
	private static class Hyracks implements QueryPlan {
		...
		public JoinMatrix generateMatrix() {
			...
			ProjectOperator projectionFirst = new ProjectOperator(new int[] {0});
			ChainOperator firstOps = new ChainOperator(projectionFirst);
			List<Integer> firstJoinKeys = readAllTupleJoinKeys(firstReader, firstOps, _ic, _map);
			Collections.sort(firstJoinKeys);
			
			CustomReader secondReader = open(_dataPath + "orders" + _extension);
			ProjectOperator projectionSecond = new ProjectOperator(new int[]{1});
			ChainOperator secondOps = new ChainOperator(projectionSecond);
			List<Integer> secondJoinKeys = readAllTupleJoinKeys(secondReader, secondOps, _ic, _map);
			Collections.sort(secondJoinKeys);
			
			ComparisonPredicate<Integer> comparison = new ComparisonPredicate<Integer>(ComparisonPredicate.EQUAL_OP);

			JoinMatrix joinMatrix = new UJMPAdapterByteMatrix(firstJoinKeys.size(), secondJoinKeys.size(), _map);
