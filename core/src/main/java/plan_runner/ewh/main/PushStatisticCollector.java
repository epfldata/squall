...
import plan_runner.ewh.algorithms.OutputShallowCoarsener;
...
import plan_runner.ewh.data_structures.FrequencyPosition;
...
public class PushStatisticCollector {
	...
	public void finalizeProcessing() {
		try{
			...
			//boolean toWriteKeyRegions = SystemParameters.getBoolean(_map, "DIP_WRITE_KEY_REGIONS");
			...
			for(TilingAlgorithm algorithm: _algorithms){
				try{
					...
					//joinMatrix = joinMatrix.getDeepCopy(); it does not work correctly
					joinMatrix = _queryPlan.generateMatrix();
					double regenerateMatrixTime = (System.currentTimeMillis() - regenerateMatrixStart)/1000.0;
					LOG.info("\nRegenerate matrix time is " + regenerateMatrixTime + "seconds.\n");
					
					//print info
					StringBuilder sb = new StringBuilder();
					sb.append("\n").append(queryName).append(" in ").append(dataPath);
					sb.append(" of size [").append(joinMatrix.getXSize()).append(", ").append(joinMatrix.getYSize()).append("]\n");
					sb.append("The number of joiners is ").append(_numLastJoiners).append("\n");
					...
					if(wp != null){
						...
						opt = new MaxAvgOptimality(joinMatrix, regions, algorithm.getWeightFunction());
					}
					
					// compute the joiner regions
					List<KeyRegion> keyRegions = generateKeyRegions(regions, joinMatrix, wrapper);
					String queryId = MyUtilities.getQueryID(_map);
					String filename = SystemParameters.getString(_map, "DIP_KEY_REGION_ROOT") + "/" + queryId;
					...
					sb.append("Final regions are: ").append(Region.toString(regions, opt, "Final"));
					sb.append("\nElapsed algorithm time is ").append(elapsed).append(" seconds.\n");
					sb.append("\n[").append(opt.toString()).append("] is ").append(opt.getOptimalityDistance()).append("\n");
					sb.append("\n=========================================================================================\n");
					_fileLogger.info(sb.toString());
	...
	public static <JAT extends Comparable<JAT>> List<KeyRegion> generateKeyRegions(List<Region> regions, JoinMatrix<JAT> joinMatrix, NumericConversion wrapper) {
		...
		for(Region region: regions){
			...
			// the upper positions of x and y are not inclusive - necessary for not missing a cell
			...
			//      position: the lowest position of the key in the region (the position absolute to the joinMatrix)
			...
			// ------------------------------
			double prob_kx1Lower = computeLowerProb(fpx1, spkx1, kx1, joinMatrix, true, wrapper);
			double prob_kx1Upper = computeUpperProb(fpx1, spkx1, kx1, joinMatrix, true, wrapper);
			
			double prob_ky1Lower = computeLowerProb(fpy1, spky1, ky1, joinMatrix, false, wrapper);
			double prob_ky1Upper = computeUpperProb(fpy1, spky1, ky1, joinMatrix, false, wrapper);
			
			double prob_kx2Lower = computeLowerProb(fpx2, spkx2, kx2, joinMatrix, true, wrapper);
			double prob_kx2Upper = computeUpperProb(fpx2, spkx2, kx2, joinMatrix, true, wrapper);
			
			double prob_ky2Lower = computeLowerProb(fpy2, spky2, ky2, joinMatrix, false, wrapper);
			double prob_ky2Upper = computeUpperProb(fpy2, spky2, ky2, joinMatrix, false, wrapper);
			// ------------------------------			
						
			// finally, creating and adding the region
			KeyRegion keyRegion = new KeyRegion(kx1, ky1, kx2, ky2, prob_kx1Lower, prob_kx1Upper, prob_ky1Lower, prob_ky1Upper, prob_kx2Lower, prob_kx2Upper, prob_ky2Lower, prob_ky2Upper,index);
			index++;
	...
	private static <JAT extends Comparable<JAT>> double computeLowerProb(FrequencyPosition fp, int sp, JAT k, JoinMatrix<JAT> joinMatrix, boolean isX, NumericConversion wrapper){
		...
		// obtain a probability that a tuple with such a key goes to this KeyRegion (Joiner)			
	...
	private static  <JAT extends Comparable<JAT>> double computeUpperProb(FrequencyPosition fp, int sp, JAT k, JoinMatrix<JAT> joinMatrix, boolean isX, NumericConversion wrapper){
		if(k.equals(wrapper.getMinValue()) || k.equals(wrapper.getMaxValue())){
			// the extreme values are always entirely included
			return 1;
		}
	...
	private static <T extends Comparable<T>> void fillMatrix(JoinMatrix joinMatrix, 
			List<T> firstKeys, List<T> secondKeys, ComparisonPredicate comparison){
		...
		LOG.info("Set tuple keys, just about to fill the matrix at " + getWallClockTime());
	...
	private static <T extends Comparable<T>> void fillOutput(JoinMatrix joinMatrix, 
			List<T> firstKeys, List<T> secondKeys, ComparisonPredicate comparison){
		...
		InputShallowCoarsener ic = new InputShallowCoarsener(numXBuckets, numYBuckets);
		...
		for(int i = 0; i < numXBuckets; i++){
			...
			for (int j = 0; j < numYBuckets; j++){
				...
				if(MyUtilities.isCandidateRegion(joinMatrix, region, comparison, _map)){
					// only candidates should be analyzed for setting to 1
					for(int x = x1; x <= x2; x++){
						for(int y = y1; y <= y2; y++){
							if (comparison.test(firstKeys.get(x), secondKeys.get(y))){
	...
	/*
	private static <T extends Comparable<T>> void fillOutputOld(JoinMatrix joinMatrix, 
			List<T> firstKeys, List<T> secondKeys, ComparisonPredicate comparison){

		int xSize = firstKeys.size();
		int ySize = secondKeys.size();

		// enter actual matrix (output tuples)
		for(int i = 0; i < xSize; i++){
	...
	*/	
	...
	private void createFileLogger(){
	    ...
	    try {  
	    	...
	    	String path = SystemParameters.getString(_map, "DIP_OPTIMALITY_LOGS") + "/optimality.info"; 
	...
	private void specifyAlgorithms(){
		...
		//-------
		// !!!! WATCH OUT: IF YOU USE TWO BSP ALGORITHMS, CHECK WHAT DO YOU DO WITH ORIGINAL JOIN_MATRIX
		_algorithms = new ArrayList<TilingAlgorithm>();
		WeightFunction wf = new WeightFunction(1, 1);
		//_algorithms.add(new OkcanCandidateInputAlgorithm(_numLastJoiners, wf, okcanP, okcanP, _map));
		//_algorithms.add(new OkcanCandidateOutputAlgorithm(_numLastJoiners, wf, okcanP, okcanP, _map));
		_algorithms.add(new OkcanExactInputAlgorithm(_numLastJoiners, wf, okcanP, okcanP, _map));
		_algorithms.add(new OkcanExactOutputAlgorithm(_numLastJoiners, wf, okcanP, okcanP, _map));
		ShallowCoarsener inputCoarsener = new InputShallowCoarsener(bspP, bspP);
		//_algorithms.add(new BSPAlgorithm(_map, _numLastJoiners, wf, inputCoarsener, BSPAlgorithm.COVERAGE_MODE.DENSE));
		_algorithms.add(new BSPAlgorithm(_map, _numLastJoiners, wf, inputCoarsener, BSPAlgorithm.COVERAGE_MODE.SPARSE));
		// ShallowCoarsener outputCoarsener = new OutputShallowCoarsener(bspP, bspP, wf, _map);
		//_algorithms.add(new BSPAlgorithm(_map, _numLastJoiners, wf, outputCoarsener, BSPAlgorithm.MODE.DENSE));
		//_algorithms.add(new BSPAlgorithm(_map, _numLastJoiners, wf, outputCoarsener, BSPAlgorithm.MODE.SPARSE));
		ShallowCoarsener inputOutputCoarsener = new InputOutputShallowCoarsener(bspP, bspP, wf, _map);
		_algorithms.add(new BSPAlgorithm(_map, _numLastJoiners, wf, inputOutputCoarsener));
	}
	
	private void chooseQueryPlan(){
        String queryName = SystemParameters.getString(_map, "DIP_QUERY_NAME");
        _queryPlan = null;
	    if(queryName.equalsIgnoreCase("hyracks") || queryName.equalsIgnoreCase("theta_hyracks")){
	    	_queryPlan = new Hyracks(_map);
	    	_numLastJoiners = SystemParameters.getInt(_map, "JOINERS_CUSTOMER_ORDERS_PAR");
	    }else if(queryName.equalsIgnoreCase("theta_lines_self_join")){
	    	_queryPlan = new ThetaLineitemSelfJoin(_map);
	    	_numLastJoiners = SystemParameters.getInt(_map, "JOINERS_LINEITEM1_LINEITEM2_PAR");
	    	...
	    	_numLastJoiners = SystemParameters.getInt(_map, "JOINERS_REGION_NATION_SUPPLIER_LINEITEM_PAR");
	    }else if(queryName.equalsIgnoreCase("theta_tpch7_L_S_N1")){
	    	_queryPlan = new ThetaTPCH7_L_S_N1(_map);
	    	_numLastJoiners = SystemParameters.getInt(_map, "JOINERS_LINEITEM_SUPPLIER_NATION1_PAR");
	    }else{
	...
	private static class Hyracks implements QueryPlan {
		...
		private ProjectOperator _firstProject = new ProjectOperator(new int[] {0});
		private List<Integer> _secondJoinKeys = new ArrayList<Integer>();
		private ProjectOperator _secondProject = new ProjectOperator(new int[] {0});
        ...
        public void processTuple(List<String> tuple, int relationNumber){
        	if(relationNumber == 0){
        		...
        		throw new RuntimeException("Unknown relationNumber " + relationNumber);
        	}
        }
		...
		public JoinMatrix generateMatrix() {
			...
			ComparisonPredicate<Integer> comparison = 
					new ComparisonPredicate<Integer>(ComparisonPredicate.EQUAL_OP);

			JoinMatrix joinMatrix = new UJMPAdapterByteMatrix(_firstJoinKeys.size(), _secondJoinKeys.size(), _map, comparison, _ic);
	...
	private static class ThetaLineitemSelfJoin implements QueryPlan {
		...
		public JoinMatrix generateMatrix() {
			...
			(ComparisonPredicate.SYM_BAND_WITH_BOUNDS_OP, 1, _dateIntConv);

			JoinMatrix joinMatrix = new UJMPAdapterByteMatrix(_firstJoinKeys.size(), _secondJoinKeys.size(), _map, comparison,_dateIntConv);
			fillMatrix(joinMatrix, _firstJoinKeys, _secondJoinKeys, comparison);
			return joinMatrix;
		}
	...
	private static class ThetaLineitemSelfJoinInputDominated implements QueryPlan {
		private IntegerConversion _ic = new IntegerConversion();
		
		private Map _map;
		
		private List<Integer> _firstJoinKeys = new ArrayList<Integer>();
		private ProjectOperator _firstProject = new ProjectOperator(new int[] {5});
		private List<Integer> _secondJoinKeys = new ArrayList<Integer>();
		private ProjectOperator _secondProject = new ProjectOperator(new int[] {5});
	...
	private static class ThetaTPCH5_R_N_S_L implements QueryPlan {
		...
		private List<Integer> _firstJoinKeys = new ArrayList<Integer>();
		private ProjectOperator _firstProject = new ProjectOperator(new int[] {2});
		private List<Integer> _secondJoinKeys = new ArrayList<Integer>();
		private ProjectOperator _secondProject = new ProjectOperator(new int[] {1});
