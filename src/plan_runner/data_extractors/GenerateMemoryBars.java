...
public class GenerateMemoryBars {
	...
	public GenerateMemoryBars(String inPath, String[] queryNames, int[] algs) {
		_inPath = inPath;
		_outPath = inPath + "/" + queryNameToDir(queryNames[0]) + "/" + OUT_MEMORY_FILE;
		try {
	...
	private void process(String[] queryNames, int[] algs) throws Exception {
		...
		for(int i=0; i < queryNames.length; i++){
			...
			for (int j = 0; j < algs.length; j++) {
				String filePath = _inPath + "/" + queryDir + "/" + getFolderName(algs[j])+ "/" + IN_MEMORY_FILE;
 ...
 */
	public GenerateMemoryBars(String inPath, String queryName, int[] maps, int[] algs) {
	...
	private void processOptimal(int[] maps, int[] algs) throws Exception {
		...
		//the order is important
		out.write("Mapping," + getLegend(STATIC_NAIVE)+","+getLegend(DYNAMIC)+","+getLegend(STATIC_OPT)+","+getLegend(M8_8)+"\n");
		...
		for(int i=0; i < maps.length; i++){
			...
			out.write(mapName);
			for (int j = 0; j < algs.length; j++) {
				String filePath = _inPath + "/" + getFolderName(maps[i]) + "/" + getFolderName(algs[j])+ "/" + IN_MEMORY_FILE;
				System.out.println(filePath);
	...
	public static void main(String[] args) throws Exception{
		...
		// non-scalability
		new GenerateMemoryBars(current+"/VLDBPaperLatex/Results/csv/", queries, algs);
		new GenerateMemoryBars(current+"/VLDBPaperLatex/Results/csv/mappings", BAND_INPUT, maps, algs);
	}
