...
public class ThetaTPCH8Plan {
   ...
   public ThetaTPCH8Plan(String dataPath, String extension, Map conf){
	   ...
	   int Theta_JoinType=ThetaQueryPlansParameters.getThetaJoinType(conf);
        ...
        //TODO
        Component R_N_Cjoin=null;
		
		if(Theta_JoinType==0){
			R_N_Cjoin = new ThetaJoinStaticComponent(
					R_Njoin,
	                relationCustomer,
	                _queryPlan).addOperator(new ProjectOperator(new int[]{2}))
	                           .setHashIndexes(Arrays.asList(0))
	                           .setJoinPredicate(R_N_C_comp);
		}
		else if(Theta_JoinType==1){
			R_N_Cjoin = new ThetaJoinDynamicComponentAdvisedEpochs(
					R_Njoin,
	                relationCustomer,
	                _queryPlan).addOperator(new ProjectOperator(new int[]{2}))
	                           .setHashIndexes(Arrays.asList(0))
		...
		if(Theta_JoinType==0){
			S_Njoin = new ThetaJoinStaticComponent(
					relationSupplier,
	                relationNation2,
	                _queryPlan).addOperator(new ProjectOperator(new int[]{0, 3}))
	                           .setHashIndexes(Arrays.asList(0))
	                           .setJoinPredicate(S_N2_comp);
		}
		else if(Theta_JoinType==1){
			S_Njoin = new ThetaJoinDynamicComponentAdvisedEpochs(
					relationSupplier,
	                relationNation2,
	                _queryPlan).addOperator(new ProjectOperator(new int[]{0, 3}))
	                           .setHashIndexes(Arrays.asList(0))
...
//        AggregateCountOperator agg= new AggregateCountOperator(conf);
		...
		if(Theta_JoinType==0){
			P_Ljoin = new ThetaJoinStaticComponent(
					relationPart,
	                relationLineitem,
	                _queryPlan).addOperator(new ProjectOperator(new int[]{1,3,4}))
	                           .setHashIndexes(Arrays.asList(0))
...
//	                           .addOperator(agg)
       ...
       //-------------------------------------------------------------------------------------
        ...
        Component P_L_Ojoin=null;
		
		if(Theta_JoinType==0){
			P_L_Ojoin = new ThetaJoinStaticComponent(
					P_Ljoin,
	                relationOrders,
	                _queryPlan).addOperator(new ProjectOperator(new int[]{1,2,4,5}))
	                           .setHashIndexes(Arrays.asList(0))
	                           .setJoinPredicate(P_L_O_comp);
		}
		else if(Theta_JoinType==1){
			P_L_Ojoin = new ThetaJoinDynamicComponentAdvisedEpochs(
					P_Ljoin,
	                relationOrders,
	                _queryPlan).addOperator(new ProjectOperator(new int[]{1,2,4,5}))
	                           .setHashIndexes(Arrays.asList(0))
        ...
        Component S_N_P_L_Ojoin=null;
		
		if(Theta_JoinType==0){
			S_N_P_L_Ojoin = new ThetaJoinStaticComponent(
					S_Njoin,
	                P_L_Ojoin,
	                _queryPlan).addOperator(new ProjectOperator(new int[]{1,3,4,5}))
	                           .setHashIndexes(Arrays.asList(2))
	                           .setJoinPredicate(S_N_P_L_O_comp);
		}
		else if(Theta_JoinType==1){
			S_N_P_L_Ojoin = new ThetaJoinDynamicComponentAdvisedEpochs(
					S_Njoin,
	                P_L_Ojoin,
	                _queryPlan).addOperator(new ProjectOperator(new int[]{1,3,4,5}))
	                           .setHashIndexes(Arrays.asList(2))
        ...
        Component R_N_C_S_N_P_L_Ojoin=null;
		
		if(Theta_JoinType==0){
			R_N_C_S_N_P_L_Ojoin = new ThetaJoinStaticComponent(
					R_N_Cjoin,
	                S_N_P_L_Ojoin,
			...
			R_N_C_S_N_P_L_Ojoin = new ThetaJoinDynamicComponentAdvisedEpochs(
					R_N_Cjoin,
	                S_N_P_L_Ojoin,
	                _queryPlan).addOperator(agg)
	                .setJoinPredicate(R_N_C_S_N_P_L_O_comp)
