...
public class GenerateOptimalMappings {
	...
	public GenerateOptimalMappings(long numberOfRelation1, long numberOfRelation2, int multFactor, int initialN, int initialM) {
		_multFactor=multFactor;
		_numberOfRelation1= numberOfRelation1;
		_numberOfRelation2= numberOfRelation2;
		n=initialN; m=initialM;
	...
	private void process(){
		...
		while(_currentRelationCount1<_numberOfRelation1 || _currentRelationCount2<_numberOfRelation2){
			
			if(currentRelationPointer==2){
				
				if(_currentRelationCount1<_numberOfRelation1)
					nextToStop= (_currentRelationCount1*_multFactor) - _currentRelationCount2;
				else
...
//						System.out.println(_currentRelationCount1+","+_currentRelationCount2);
						//check for change !!
						int[] newMapping=findOptimal(_currentRelationCount1, _currentRelationCount2, n, m);
						if(newMapping[0]>0){
							//change triggered
							numOfOptimalMigrations++;
							n= newMapping[0]; m= newMapping[1];
							System.out.println("Changing mapping at: "+_currentRelationCount1+","+_currentRelationCount2+" to ("+n+","+m+")");
						}
					}
					else {currentRelationPointer=1;  break;}
				}
				currentRelationPointer=1;
			}
			else if(currentRelationPointer==1){
				if(_currentRelationCount2<_numberOfRelation2)
					nextToStop=( _currentRelationCount2*_multFactor) -_currentRelationCount1;
				else
...
//						System.out.println(_currentRelationCount1+","+_currentRelationCount2);
					...
					else {currentRelationPointer=2;  break;}
				}
				currentRelationPointer=2;
			}
		}
		
		System.out.println("Finalized at: "+_currentRelationCount1+","+_currentRelationCount2+" to ("+n+","+m+")"+" with number of optimal migrations "+numOfOptimalMigrations);
	...
	public static void main(String[] args) {
		GenerateOptimalMappings g = new GenerateOptimalMappings(10000000, 10000000, 4, 4, 4);
