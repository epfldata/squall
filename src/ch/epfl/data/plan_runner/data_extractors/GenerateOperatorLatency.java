...
public class GenerateOperatorLatency {
	...
	private static final double TOP_RESULTS = 0.74; // the relative TOP latency files are analyzed
	...
	private double getAvgLatency(String dirPath) throws IOException{
		...
		System.out.println("There are " + size + " workers which reported on latency." +
				" We pick " + cutSize +" of them with minimal latency.");
	...
	private List<FileLatency> getFileLatencies(String dirPath) throws IOException{
			...
			for (int j = 1; j < 23; j++) {
				...
				if (f.exists()) {
					...
					if(latency != INVALID){
						...
						System.out.println("File " + suffix + " has latency " + latency + "ms. It sent " + tuples + " tuples.");
						FileLatency fl = new FileLatency(suffix, latency, tuples);
	...
	private long getSentTuples(String fullPath) throws IOException{
		...
		if(result == INVALID){
			throw new RuntimeException("No information of number of sent tuples for " + fullPath);
		}
		return result;
	}
	...
	public static void main(String[] args) throws IOException{	
		...
		String fullPath = working + "cyclone_res/R7_squall_latency/squall_1k_store/";
		String avgLatency = "";
		...
		//Q5
		out.write(THETA_TPCH5 + ",");
		avgLatency = String.valueOf(generator.getAvgLatency(fullPath + "/10G_z4_static_naive_theta_tpch5_R_N_S_L"));
		out.write(avgLatency + ",");
		avgLatency = String.valueOf(generator.getAvgLatency(fullPath + "/10G_z4_dynamic_theta_tpch5_R_N_S_L"));
		out.write(avgLatency + ",");
		avgLatency = String.valueOf(generator.getAvgLatency(fullPath + "/10G_z4_static_opt_theta_tpch5_R_N_S_L"));
		out.write(avgLatency + "\n");
		
		//Q7
		out.write(THETA_TPCH7 + ",");
		avgLatency = String.valueOf(generator.getAvgLatency(fullPath + "/10G_z4_static_naive_theta_tpch7_L_S_N1"));
		out.write(avgLatency + ",");
		avgLatency = String.valueOf(generator.getAvgLatency(fullPath + "/10G_z4_dynamic_theta_tpch7_L_S_N1"));
		out.write(avgLatency + ",");		
		avgLatency = String.valueOf(generator.getAvgLatency(fullPath + "/10G_z4_static_opt_theta_tpch7_L_S_N1"));
		out.write(avgLatency + "\n");
		
		//BNCI
		out.write(BAND_INPUT + ",");
		avgLatency = String.valueOf(generator.getAvgLatency(fullPath + "/10G_uniform_static_naive_bnci"));
		out.write(avgLatency + ",");
		avgLatency = String.valueOf(generator.getAvgLatency(fullPath + "/10G_uniform_dynamic_bnci"));		
		out.write(avgLatency + ",");
		avgLatency = String.valueOf(generator.getAvgLatency(fullPath + "/10G_uniform_static_opt_bnci"));
		out.write(avgLatency + "\n");
		...
		*/
		...
		String path = working + "cyclone_res/R7_squall_latency/dynamic_storm_16K_store_less_src/";
		...
		for(String suffix: suffixes){
			avgLatency = String.valueOf(generator.getAvgLatency(path + "/" + suffix));
			System.out.println("\n\nAverage latency for " + suffix + " is " + avgLatency + ".");
		}
