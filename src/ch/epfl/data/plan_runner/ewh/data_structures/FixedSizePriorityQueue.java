...
public class FixedSizePriorityQueue<T> extends PriorityQueue<T>{
	 ...
	 */
	public FixedSizePriorityQueue(final int maxSize, final Comparator<? super T> comparator) {
		super(maxSize, comparator); // initialCapacity is set to maxSize, as it will always reach this size
		if (maxSize <= 0) {
	...
	public boolean add(final T e) {
		...
		if (_maxSize <= size()) {
			...
			if (super.comparator().compare(e, firstElm) < 1) {
				// e.priority is less than the smallest priority in the queue; not added
				return false;
			} else {
				// remove the element with the smallest priority to provide space for the new element
				poll();
			}
		}
		super.add(e);
