...
// Join Attribute Type
public class UJMPAdapterIntMatrix<JAT extends Comparable<JAT>> extends JoinMatrix<JAT>{
	...
	public UJMPAdapterIntMatrix(int xSize, int ySize, Map map, ComparisonPredicate<JAT> cp, TypeConversion<JAT> wrapper) {
	...
	public UJMPAdapterIntMatrix(int numNonZeros, int xSize, int ySize) {
		...
		_ujmpMatrix = new DefaultSparseIntMatrix(_capacity, new long[]{xSize, ySize}); // The first argument is MAX_SIZE
	}
	...
	public UJMPAdapterIntMatrix(String matrixPath, String matrixName){
		...
		try {
			...
			_ujmpMatrix = new DefaultSparseIntMatrix(ImportMatrixSPARSECSV.fromFile(new File(path)));
		} catch (Exception exc) {
	...
	public UJMPAdapterIntMatrix<JAT> getDeepCopy() {
		if(_capacity == -1){
			...
			throw new RuntimeException("Cannot make a deep copy of UJMPAdapterIntMatrix which created from a file!");
		}
	...
	public static void main (String[] args){
		UJMPAdapterIntMatrix<Integer> joinMatrix = new UJMPAdapterIntMatrix<Integer>(100, 100);
