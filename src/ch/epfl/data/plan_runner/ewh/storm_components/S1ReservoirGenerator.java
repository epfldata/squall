...
public class S1ReservoirGenerator<JAT extends Number & Comparable<JAT>> extends BaseRichBolt implements StormEmitter{
	private static final long serialVersionUID = 1L;
	private static Logger LOG = Logger.getLogger(S1ReservoirGenerator.class);
	
	private StormEmitter _d2Combiner, _s1Source;
	private final String _d2CombinerIndex, _s1SourceIndex, _componentIndex, _partitionerIndex;
	...
	private boolean _isEWHS1Histogram; //receives either d2 or d2equi from D2Combiner
	private static final boolean _isSinglePassD2Equi = true; // Opt2: can be set to true only for _isEWHS1Histogram = true
	                                                         // reduces number of accesses to d2 by computing d2 from d2_equi in a single pass
                                                             // if turned off, check "If Opt2 is not used"	
	
	private static final boolean _isSinglePassDebug = false; // can be set to true only if _isSinglePassD2Equi is true; 
	                                                         // checks whether the result is correct by using more space: don't use it in production (only for testing purposes) 
	private Map<JAT, Integer> _d2OldDebug = new HashMap<JAT, Integer>(); // key, multiplicity: only if _isSinglePassDebug = true
	//private TreeMap<JAT, Integer> _d2Equi = new TreeMap<JAT, Integer>(); // key, multiplicity: used only when _isSinglePassD2Equi is set
	private TreeMap<JAT, Integer> _d2BeginEnd = new TreeMap<JAT, Integer>(); // key, multiplicity: used only when _isSinglePassD2Equi is set
	private int _band;
	...
	private TreeMap<JAT, Integer> _d2 = new TreeMap<JAT, Integer>(); // Opt1 is JAT instead of String key (key, multiplicity). If Opt2 is not used, use HashMap 
	private Map<String, Integer> _r1 = new HashMap<String, Integer>(); //key, multiplicity 
	private PriorityQueue<KeyPriorityProbability> _reservoir;
	
	private Random _rndGen = new Random();
	
	public S1ReservoirGenerator(StormEmitter d2Reduce, StormEmitter s1Source, String componentName, String partitionerName,
			boolean isEWHS1Histogram, // receives either d2 or d2equi from D2Combiner
		...
		_d2CombinerIndex = String.valueOf(allCompNames.indexOf(d2Reduce.getName()));
		_s1Source = s1Source;
		_s1SourceIndex = String.valueOf(allCompNames.indexOf(s1Source.getName()));
		_partitionerIndex = String.valueOf(allCompNames.indexOf(partitionerName));
		...
		final int parallelism = SystemParameters.getInt(conf, componentName + "_PAR");
		...
		if(!_isEWHS1Histogram){
			currentBolt = MyUtilities.attachEmitterHash(SystemParameters.D2_TO_S1_STREAM, conf, null, currentBolt, d2Reduce);
			// acks from d2Reduce and all the data from s1Source
			currentBolt = MyUtilities.attachEmitterHash(conf, null, currentBolt, d2Reduce, s1Source);
		}else{
			HistogramType dstHistType = HistogramType.S1_RES_HIST;
			// if this histogram exists, the other must exist as well
			if(!SystemParameters.getBooleanIfExist(_conf, HistogramType.D2_COMB_HIST.readConfEntryName())){
				throw new RuntimeException("If S1Reservoir Histogram is set, D2Combiner histogram must also be set in the config file!");
			}
			HistogramType srcHistType = HistogramType.D2_COMB_HIST;
			currentBolt = MyUtilities.attachEmitterFilteredRangeMulticast(SystemParameters.D2_TO_S1_STREAM, conf, comparison, wrapper, dstHistType, srcHistType, 
					d2Reduce.getName(), currentBolt, d2Reduce);

			// acks from d2Reduce
			currentBolt = MyUtilities.attachEmitterHash(conf, null, currentBolt, d2Reduce);
			
			//all the data from s1Source
			currentBolt = MyUtilities.attachEmitterRange(_conf, _wrapper, dstHistType, currentBolt, s1Source);
		}

		if(MyUtilities.isAutoOutputSampleSize(conf)){
			// from Partitioner we receive the size of output sample
			currentBolt = MyUtilities.attachEmitterBroadcast(SystemParameters.FROM_PARTITIONER, currentBolt, partitionerName);
		}
		
		// manually setting reservoir size
		if(!MyUtilities.isAutoOutputSampleSize(conf)){
			int outputSampleSize = MyUtilities.computeManualSampleSize(conf, _firstNumOfBuckets, _secondNumOfBuckets);
	...
	private void constructReservoir(int outputSampleSize){
		_reservoir = new FixedSizePriorityQueue<KeyPriorityProbability>(outputSampleSize, new KeyPriorityProbability.KeyPriorityComparator());
	}
		...
		if(inputComponentIndex.equals(_d2CombinerIndex)){
			...
			if(!_isEWHS1Histogram){
				...
				if(_isSinglePassD2Equi){
					// for now update only _d2Equi, and later build _d2 in a single pass using createD2OutOfD2Equi
	...
	private void addMultiplicityJoin(Map<JAT, Integer> stats, JAT key, int mult){
		...
		for(JAT joinableKey: joinableKeys){
			//String joinableStrKey = MyUtilities.toSpecialString(joinableKey, _wrapper); not needed anymore
			addMultiplicity(stats, joinableKey, mult);
		}
	}
	
	// at D2Combiner, one machine which is assigned that very key will get all the joinable set from R2 (R2->D2Combiner uses RangeMulticast)
	...
	private void finalizeProcessing(){
		...
		// creating reservoir
		if(!_d2.isEmpty()){
			// no need to iterate over _r1 if _d2 is empty: _d2 could be empty as equi-depth on R1 and on R2 can vastly differ (see Bicd)
			...
			for (Map.Entry<String, Integer> entry: _r1.entrySet()) {
				...
				if((d2Mult = getMultiplicity(_d2, r1Key)) > 0){
					// there is no joinable tuple from d2, so the weight and priority is 0
					double d2KeyProbability = ((double)d2Mult)/r2RelationSize;
					double power = 1.0 / d2Mult;
					for(int i = 0; i < r1Mult; i++){
						// there may be multiple rows from R1 with the same key
						//   for each of them a separate random generator invocation
						double priority = Math.pow(_rndGen.nextDouble(), power);
						KeyPriorityProbability keyPriority = new KeyPriorityProbability(r1StrKey, priority, d2KeyProbability);
						_reservoir.add(keyPriority); // add method is changed in the implementation such that we do not violate capacity constraints
					}
		...
		LOG.info("I am sending reservoir of size " + _reservoir.size() + " (should be (at most) outputSampleSize large).");
		while((kp = _reservoir.poll()) != null){
			...
			List<String> tuple = new ArrayList<String>(Arrays.asList(strKey, strPriority, strD2KeyProbability));
			List<Integer> hashIndexes = new ArrayList<Integer>(Arrays.asList(0));
			tupleSend(SystemParameters.RESERVOIR_TO_MERGE, tuple, hashIndexes);
			if(SystemParameters.getBooleanIfExist(_conf, "DEBUG_MODE")){
				// this is just for local debugging
				LOG.info("Reservoir element in S1ReservoirGenerator is " + tuple);
			}
		}
		
		//sending total number of output tuples
		Values totalOutpuSize = MyUtilities.createTotalOutputSizeTuple(_componentIndex, _computedTotalOutputs);
	...
	private void createD2OutOfD2Equi(){
		if(!_d2BeginEnd.isEmpty()){
			...
			for(Map.Entry<JAT, Integer> entry: _d2BeginEnd.entrySet()){
				multInRange += entry.getValue(); // can be negative when exiting out of scope of a key
				entry.setValue(multInRange); // updating in place: we won't need previous values anymore
			}
			_d2 = _d2BeginEnd;
			...
			if(_isSinglePassDebug){
				...
				if(!MyUtilities.isMapsEqual(_d2, _d2OldDebug, sb)){
					throw new RuntimeException("_d2 and _d2OldDebug are not equal! " + sb.toString());
				}
			}
		}
	}
	...
	/*
	 * Old version uses _d2Equi
	private void createD2OutOfD2Equi(){
		if(!_d2Equi.isEmpty()){
			// _d2Equi could be empty as equi-depth on R1 and on R2 can vastly differ (see Bicd)
			JAT firstKey = _d2Equi.firstKey();
			JAT lastKey = _d2Equi.lastKey();
			int multInRange = 0;
			for(JAT currentKey = (JAT) _wrapper.getOffset(firstKey, -_band); 
					currentKey.compareTo((JAT) _wrapper.getOffset(lastKey, _band + 1)) < 0;
					currentKey = (JAT)_wrapper.minIncrement(currentKey)){

				// entering into range
				// check if there is a d2Equi 'band' positions to the right
				JAT enteringKey = (JAT) _wrapper.getOffset(currentKey, _band);
				if (_d2Equi.containsKey(enteringKey)){
					multInRange += _d2Equi.get(enteringKey);
				}
				//exiting from range
				// check if there is a d2Equi 'band + 1' positions to the left
				JAT exitingKey = (JAT) _wrapper.getOffset(currentKey, -(_band + 1));
				if(_d2Equi.containsKey(exitingKey)){
					multInRange -= _d2Equi.get(exitingKey);
				}

				if(multInRange > 0){
					//writing this into data structure
					_d2.put(currentKey, multInRange);
				}else{
					//let's move to the first next non-zero position
					// +1 because we will increment at the end of this iteration
					// we can use method higherKey because all the keys are unique in _d2Equi
					JAT nextPositivePos = (JAT) _wrapper.getOffset(_d2Equi.higherKey(currentKey), -(_band + 1)); 
					if(currentKey.compareTo(nextPositivePos) < 0){
						currentKey = nextPositivePos;
					}
				}
			}
	...
	public void prepare(Map map, TopologyContext tc, OutputCollector collector) {
		...
		_numRemainingParents = MyUtilities
				.getNumParentTasks(tc, Arrays.asList(_d2Combiner, _s1Source));
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		if(_hierarchyPosition == StormComponent.FINAL_COMPONENT){
			...
			declarer.declareStream(SystemParameters.RESERVOIR_TO_MERGE, new Fields(outputFieldsMerge));
	...
	protected boolean processFinalAck(List<String> tuple, Tuple stormTupleRcv) {
		if (MyUtilities.isFinalAck(tuple, _conf)) {
			...
			MyUtilities.processFinalAckCustomStream(SystemParameters.PARTITIONER, _numRemainingParents, StormComponent.INTERMEDIATE, _conf,
					stormTupleRcv, _collector);
			return true;
		}
		return false;
