...
public class D2CombinerBolt<JAT extends Number & Comparable<JAT>> extends BaseRichBolt implements StormEmitter{
	private static final long serialVersionUID = 1L;
	private static Logger LOG = Logger.getLogger(D2CombinerBolt.class);
	
	private StormEmitter _d2Source;
	private String _d2SourceIndex, _s1ReservoirMergeIndex = null, _componentIndex;
	...
	private boolean _isEWHS1Histogram; // if true, only d2_equi is computed in D2CombinerBolt
	
	private Map<JAT, Integer> _d2 = new HashMap<JAT, Integer>(); // key, multiplicity
	private Map<JAT, Integer> _d2equi = new HashMap<JAT, Integer>(); // key, multiplicity
	...
	// state
	private enum STATE {PRODUCING_D2, PRODUCING_SAMPLE_OUTPUT};
	private STATE _state = STATE.PRODUCING_D2;
	
	public D2CombinerBolt(StormEmitter d2Source, String s1ReservoirMergeName, String componentName, boolean isFirstD2, 
			NumericConversion<JAT> wrapper, ComparisonPredicate comparison, boolean isEWHS1Histogram, // if true, only d2_equi is computed in D2CombinerBolt
		...
		_d2Source = d2Source;
		_d2SourceIndex = String.valueOf(allCompNames.indexOf(d2Source.getName()));
		_s1ReservoirMergeIndex = String.valueOf(allCompNames.indexOf(s1ReservoirMergeName));
		...
		currentBolt = MyUtilities.attachEmitterRangeMulticast(conf, comparison, _wrapper, histType, currentBolt, d2Source);
		currentBolt = MyUtilities.attachEmitterRange(_conf, _wrapper, histType, currentBolt, s1ReservoirMergeName);
	...
	private void processNonLastTuple(String inputComponentIndex, String sourceStreamId, List<String> tuple) {
		if(inputComponentIndex.equals(_d2SourceIndex)){
			...
			String strOppositeKey = MyUtilities.toSpecialString(oppositeKey, _wrapper);
			//output
			List<String> outputSampleTuple;
			if(_isFirstD2){
				outputSampleTuple = new ArrayList<String>(Arrays.asList(strOppositeKey, strS1Key)); 
			}else{
				outputSampleTuple = new ArrayList<String>(Arrays.asList(strS1Key, strOppositeKey)); 
			}
			...
			tupleSend(SystemParameters.PARTITIONER, outputSampleTuple, hashIndexes);
		}else{
			throw new RuntimeException("Unsupported source component index " + inputComponentIndex);
		}
	}
	...
	// increasing the multiplicity for each key which is joinable with key (arg) from the opposite relations
	...
	private JAT getOppositeKey(JAT s1Key) {
		...
		// these collections are of the same size; contains keys with multiplicity > 0
	    ...
	    // each joinable key has a probability to be included [previous.probabilityEnd, probabilityEnd)
		int multiplicitySum = 0;
		for(int multiplicity: multiplicities){
			multiplicitySum += multiplicity;
			double probabilityEnd = ((double)(multiplicitySum)) / totalMultiplicity;
			probabilities.add(probabilityEnd);
		}
		
		// choose one tuple among the joinable ones proportionally to their multiplicity
		...
		for(int i = 0; i < probabilities.size(); i++){
			...
			//LOG.info("Probability range is [" + probabilityBeg + ", " + probabilityEnd + ")");
	...
	private void finalizeProcessing(){
		if(_state == STATE.PRODUCING_D2){
			...
			LOG.info("Received _r2NumReceivedTuples = " + _r2NumReceivedTuples + " tuples.");
			...
			for (Map.Entry<JAT, Integer> entry: mapToSend.entrySet()) {
				...
				List<String> tuple = new ArrayList<String>(Arrays.asList(strKey, strMult));
	...
	public void prepare(Map map, TopologyContext tc, OutputCollector collector) {
		...
		_numRemainingParents = MyUtilities
				.getNumParentTasks(tc, Arrays.asList(_d2Source));
	}
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		if(_hierarchyPosition == StormComponent.FINAL_COMPONENT){
			...
			declarer.declareStream(SystemParameters.D2_TO_S1_STREAM, new Fields(outputFields));
			
			// default stream (used only for ACKS) ; used only for S1Reservoir component
			// the reason to avoid DATA_STREAM is to allow to send EOF to different children at different moment
			...
			declarer.declareStream(SystemParameters.DATA_STREAM, new Fields(outputFieldsDef));
