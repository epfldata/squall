...
public class InputOutputShallowCoarsener extends OutputShallowCoarsener{
	private static Logger LOG = Logger.getLogger(InputOutputShallowCoarsener.class);
	
	public InputOutputShallowCoarsener(int numXBuckets, int numYBuckets, WeightFunction wf, Map map, ADJUST_MODE amode, ITER_MODE imode) {
		super(numXBuckets, numYBuckets, wf, map, amode, imode);
	}

	public InputOutputShallowCoarsener(int numXBuckets, int numYBuckets, WeightFunction wf, Map map) {
	...
	protected double computeMaxGridCellWeight(){	
		...
		int halfPerimeter = _originalMatrix.getXSize() / _numXBuckets + _originalMatrix.getYSize() / _numYBuckets;
		
		double result = MAX_WEIGHT_BADNESS * (2 + EPSILON) * _wf.getWeight(halfPerimeter, frequency);
	...
	protected double getWeightEmpty0(Region region){
		// too expensive and does not yield better partitioning than only "return _wp.getWeight(region);" (tried on one example though):
		if (_wp.getFrequency(region) == 0 && !MyUtilities.isCandidateRegion(_originalMatrix, region, _originalMatrix.getComparisonPredicate(), _map)){
		//if (_wp.getFrequency(region) == 0){
			// input can be arbitrarily large if it contains no output
			//  Would it be an improvement if we assign non-zero weights for empty regions which are candidates?
			return 0;
		}else{
			return _wp.getWeight(region);
		}
	...
	public String toString(){
		return "InputOutputBinaryCoarsener [numXBuckets = " + _numXBuckets + ", numYBuckets = " + _numYBuckets + "]"; 
	}
