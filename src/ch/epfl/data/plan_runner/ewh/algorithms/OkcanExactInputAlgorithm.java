...
public class OkcanExactInputAlgorithm extends OkcanAlgorithm{
	...
	public OkcanExactInputAlgorithm(int j, WeightFunction wf, int numXBuckets, int numYBuckets, Map map){
	...
	public double getWeight(Region coarsenedRegion) {
		...
		Region originalRegion = getCoarsener().translateCoarsenedToOriginalRegion(coarsenedRegion);
	...
	protected int getWeightLowerBound(JoinMatrix coarsenedMatrix, int numOfRegions) {
		/* old version
		int numOutputs = coarsenedMatrix.getTotalNumOutputs();
		return (int)(2 * Math.sqrt(numOutputs/numOfRegions)); // we implied that JoinMatrix is 0/1, which is not necessarily the case
		*/
		//new version
