...
public class StormDstTupleStorageJoin extends StormJoinerBoltComponent {
	...
	protected void applyOperatorsAndSend(Tuple stormTupleRcv, List<String> tuple,
		...
		*/
		...
		// Measuring latency from the previous component, and taking into account all the tuples in the batch
		if (MyUtilities.isSending(getHierarchyPosition(), _aggBatchOutputMillis)) {
			long timestamp = 0;
			if (MyUtilities.isCustomTimestampMode(getConf()))
	...
	public void execute(Tuple stormTupleRcv) {
		...
		//short circuit that this is a window configuration
		 if (_windowSize>0 & MyUtilities.isTickTuple(stormTupleRcv)) { 
			 purgeStaleStateFromWindow();
				...
				if(storedTimestamp< (_latestTimeStamp-_GC_PeriodicTickSec)){ //delete
					...
					for (int i = 0; i < _firstRelationIndexes.size(); i++)
						if (typesOfValuesToIndex.get(i) instanceof Integer)
							...
							try {
								_firstRelationIndexes.get(i).remove(row_id, _convDateFormat.parse(valuesToIndex.get(i)));
							} catch (final ParseException e) {
								throw new RuntimeException("Parsing problem in StormThetaJoin.removingIndexes "
										+ e.getMessage());
							}
						else if (typesOfValuesToIndex.get(i) instanceof String)
			...
			if(storedTimestamp< (_latestTimeStamp-_GC_PeriodicTickSec)){ //delete
				...
				for (int i = 0; i < _secondRelationIndexes.size(); i++)
					if (typesOfValuesToIndex.get(i) instanceof Integer)
						...
						try {
							_secondRelationIndexes.get(i).remove(row_id, _convDateFormat.parse(valuesToIndex.get(i)));
						} catch (final ParseException e) {
							throw new RuntimeException("Parsing problem in StormThetaJoin.removingIndexes "
									+ e.getMessage());
						}
